...
from .top_transformer import BasicLayer
...
class HAIM(nn.Module):
    def __init__(self, in_channel,out_channel):
        ...
        self.relu = nn.ReLU(True)
        self.rgb_branch1 = BasicConv2d(in_channel, out_channel, 3, padding=1, dilation=1)
        self.rgb_branch2 = BasicConv2d(in_channel, out_channel, 3, padding=3, dilation=3)
        self.rgb_branch3 = BasicConv2d(in_channel, out_channel, 3, padding=5, dilation=5)
        self.rgb_branch4 = BasicConv2d(in_channel, out_channel, 3, padding=7, dilation=7)

        self.d_branch1 = BasicConv2d(in_channel, out_channel, 3, padding=1, dilation=1)
        self.d_branch2 = BasicConv2d(in_channel, out_channel, 3, padding=3, dilation=3)
        self.d_branch3 = BasicConv2d(in_channel, out_channel, 3, padding=5, dilation=5)
        self.d_branch4 = BasicConv2d(in_channel, out_channel, 3, padding=7, dilation=7)

        self.rgb_branch1_sa = SpatialAttention1()
        self.rgb_branch2_sa = SpatialAttention1()
        self.rgb_branch3_sa = SpatialAttention1()
        self.rgb_branch4_sa = SpatialAttention1()

        self.rgb_branch1_ca = ChannelAttention1(out_channel)
        self.rgb_branch2_ca = ChannelAttention1(out_channel)
        self.rgb_branch3_ca = ChannelAttention1(out_channel)
        self.rgb_branch4_ca = ChannelAttention1(out_channel)

        self.r_branch1_sa = SpatialAttention1()
        self.r_branch2_sa = SpatialAttention1()
        self.r_branch3_sa = SpatialAttention1()
        self.r_branch4_sa = SpatialAttention1()

        self.r_branch1_ca = ChannelAttention1(out_channel)
        self.r_branch2_ca = ChannelAttention1(out_channel)
        self.r_branch3_ca = ChannelAttention1(out_channel)
        self.r_branch4_ca = ChannelAttention1(out_channel)

        self.ca = ChannelAttention1(in_channel)
...
class zzq(nn.Module):    #很多模块的使用卷积层都是以其为基础，论文中的BConvN
    def __init__(self):
        super(zzq, self).__init__()
        
        # self.conv_66 = BasicConv2d(256,64,1)
        self.conv_77 = BasicConv2d(512,32,1)
        self.conv_88 = BasicConv2d(1024,32,1)
        self.conv_99 = BasicConv2d(2048,64,1)
        # self.conv_999 = BasicConv2d(256,128,1)
        # self.conv_88 = BasicConv2d(channel, out_channel=21, 3, padding=1)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x2, x3, x4): # 传入 x1 (512,44,44) x2 (1024,22,22) x3(2048,11,11)
       
        # x1 = self.conv_66(x1)
        x2 = self.conv_77(x2) # 512 - 32  weidu 44
        x3 = self.conv_88(x3) # 1024 - 64  weidu 22
        x4 = self.conv_99(x4) # 2048 - 128 weidu 11
        
        # print(x2.shape)
        # print(x3.shape)
        # print(x4.shape)
        # x = F.avg_pool2d(x,8,8)
        # x1 = F.avg_pool2d(x1,8,8) 
        x2 = F.avg_pool2d(x2,4,4)  # 1,1
        x3 = F.avg_pool2d(x3,2,2) # 1,1
        
      
        out = torch.cat([ x2, x3, x4], dim=1)
        # out = self.conv_999(out)
...
class BBSNet(nn.Module):    #网络框架
    def __init__(self, channel=32):
        ...
        self.dap =  DAPPM(2048, 128, 32) #
        self.dap1 =  DAPPM(64, 32, 32)
        self.dap2 =  DAPPM(256, 32, 32)
        self.dap3 =  DAPPM(512, 64, 32)
        self.dap4 =  DAPPM(1024, 64, 32)
        #upsample function
        ...
        self.haim_5 = HAIM(2048,512)
        self.haim_4 = HAIM(1024,256)
        self.haim_3 = HAIM(512,128)
        self.haim_2 = HAIM(256,64)
        self.haim_1 = HAIM(64,16)
        ...
        self.conv_999 = BasicConv2d(128, 32, 1)
        self.zzq1 = zzq()
    ...
    def forward(self, x, x_depth):
        ...
        x1_en = self.haim_1(x, x_depth)   # 5 ,64, 88, 88
        ...
        x4_en = self.haim_4(x3_1, x3_1_depth)
        ...
        x5_en = self.haim_5(x4_1, x4_1_depth)
        ...
        x4_4_4 = x4_1 + temp + x5_en
        x4_1= x4_1+temp                                        #第五层DEM
        #layer4_1 merge end
        
        x4_1_2 = self.zzq1(x2_2_2,x3_3_3,x4_4_4)
        
        x4_1_2 = self.trans(x4_1_2)
        #produce initial saliency map by decoder1
        x3_1_2 = self.upsample2(x4_1_2)
        x3_1_2 = self.conv_999(x3_1_2)

        x4_1 = self.rfb4_1(x4_4_4)
        # print(x4_4_4.shape)
        # print(x4_1_2.shape)
        x4_1 = x4_1 + x4_1_2
        # print(x4_4_4.shape)
        x4_1 = self.conv_999(x4_1)
        x3_1 = self.rfb3_1(x3_3_3)
        x3_1 = x3_1 + x3_1_2
        # x2_1 = self.rfb2_1(x2_2_2)                                 #高层次网络经历CGM
        # x1_1 = self.rfb1_2(x1_1_1)                                    #低层次网络经历CGM
        # x1_2 = self.rfb0_2(x9)
        x2_1 = self.dap3(x2_2_2)                                 #高层次网络经历CGM
        x1_1 = self.dap2(x1_1_1)                                    #低层次网络经历CGM
        x1_2 = self.dap1(x9)
